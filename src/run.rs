...
mod tests {
    ...
    fn shell(script: &str) -> CommandSpec {
        if cfg!(windows) {
            CommandSpec::new("cmd").args(["/D", "/S", "/C", script])
        } else {
    ...
    #[test]
    fn captures_only_the_configured_output_tail() {
        let spec = shell("echo 123456789").capture_limits(CaptureLimits::new(6, 4));
    ...
    #[test]
    fn exit_policy_controls_nonzero_results() {
        let allowed = shell("echo diagnostic 1>&2 & exit 7").exit_policy(ExitPolicy::AllowFailure);
        let output = run(&allowed).unwrap();
        assert_eq!(output.return_code(), Some(7));

        let checked = shell("echo diagnostic 1>&2 & exit 7");
        let error = run(&checked).unwrap_err();
        assert!(matches!(error, RunError::ExitFailure { .. }));
